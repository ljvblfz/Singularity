...
procedure NucleusEntryPoint($smap:int);
  ...
  ensures  NucleusInv($S, $StackState[$S := StackRunning], $toAbs, $AbsMem, GcVars, $MemVars, $FrameVars);
...
procedure FaultHandler($_stackState:[int]StackState, $ebp:int, $esp:int, $eip:int);
  ...
  ensures  (StackStateTag($StackState[?InterruptStack]) == ?STACK_YIELDED ==>
               NucleusInv(?InterruptStack, $_stackState, $toAbs, $AbsMem, GcVars, $MemVars, $FrameVars)
            && ebp == $ebp
            && esp == $esp);
...
procedure ResetStack($s:int);
  ...
  modifies $Eip, $RET, eax, ebx, ecx, edx, esi, edi, ebp, esp;
  modifies AllGcVars, $MemVars;
  ensures  NucleusInv($S, $StackState[$s := StackEmpty], $toAbs, $AbsMem, GcVars, $MemVars, $FrameVars);
  ensures  ebp == old(ebp);
  ensures  esp == old(esp) + 4;
...
procedure YieldTo($s:int, $_stackState:[int]StackState,
  ...
  ensures  ($StackState[$s] == StackRunning ==>
               NucleusInv($s, $StackState, $toAbs, $AbsMem, GcVars, $MemVars, $FrameVars)
            && ebp == old(ebp)
            && esp == old(esp) + 4);
  ensures  ($StackState[$s] == StackYielded($ebp, $esp, $eip) ==>
               NucleusInv($s, $_stackState, $toAbs, $AbsMem, GcVars, $MemVars, $FrameVars)
            && ebp == $ebp
            && esp == $esp);
