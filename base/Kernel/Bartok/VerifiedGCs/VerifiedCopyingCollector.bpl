...
procedure scanObjectOtherVectorPointers($vt:int)
  ...
  requires $vt == $AbsMem[$r2[Tj]][1];
  requires tag($vt) == ?OTHER_VECTOR_TAG;
  requires arrayOf($vt) == ?TYPE_STRUCT && SupportedVectorStruct(arrayElementSize($vt), mask(arrayElementClass($vt)));
  modifies $r2, $GcMem, $toAbs, Tj, Tk, $gcSlice;
  modifies eax, ebx, ecx, edx, esi, edi, ebp, esp;
  ensures  CopyGcInv($freshAbs, BF, BT, HeapLo, Fi, Fk, Fl, Ti, Tj, Tj, Tk, Tk, Tl, $Time, $r1, $r2, true, $toAbs, $AbsMem, $GcMem, $gcSlice);
  ensures  RExtend(old($r2), $r2);
{
  ...
  call edi := RoLoad32(ecx + ?VT_ARRAY_ELEMENT_SIZE);
  edx := Tj;
  ...
  loop:
    ...
    assert ebx == baseWords($vt);
    assert edi == arrayElementSize($vt);
    assert 4 * esi <= ebp;
    assert TO(esi) && baseWords($vt) <= esi; // && esi <= nElems + 3;
    assert Pointer($r2, Tj, $r2[Tj]);
    assert CopyGcInv($freshAbs, BF, BT, HeapLo, Fi, Fk, Fl, Ti, Tj, Tj + 4 * numFields($r2[Tj]), Tk, Tk, Tl, $Time, $r1, $r2, true, $toAbs, $AbsMem, $GcMem, $gcSlice);
    assert RExtend(old($r2), $r2);
    assert $r2[Tj] != NO_ABS;
...
//    if (SupportedVectorStructPointer(edi, elemMask, esi - ebx)) 
    eax := esi;
    call eax := Sub(eax, ebx);
    if (edi != 8) { goto case1; }
//    if (elemMask != 17) { goto case1; }
    call eax := And(eax, 1);
    if (eax != 0) { goto skip1; }
    goto scan;
    case1:
    if (edi != 12) { goto case2; }
//    if (elemMask != 529) { goto case2; }
    ecx := 3;
    edx := 0;
    call eax, edx := Div(eax, edx, ecx);
    if (edx == 2) { goto skip1; }
    goto scan;
    case2:
    if (edi != 16) { goto case3; }
//    if (elemMask != 17) { goto case3; }
    call eax := And(eax, 3);
    if (eax != 0) { goto skip1; }
    goto scan;
    case3:
    if (edi != 20) { goto case4; }
//    if (elemMask != 81) { goto case4; }
    ecx := 5;
    edx := 0;
    call eax, edx := Div(eax, edx, ecx);
    if (edx != 4) { goto skip1; }
    goto scan;
    case4:
    if (edi != 24) { goto skip1; }
//    if (elemMask != 6636321) { goto skip1; }
    ecx := 6;
    edx := 0;
    call eax, edx := Div(eax, edx, ecx);
    if (edx == 0) { goto skip1; }
    scan:
      ...
      if (ecx > ?gcHi) { goto skip2; }
        ...
        save3 := ebp;
        save4 := ebx;
        call forwardFromspacePtr(ecx, $AbsMem[$toAbs[Tj]][esi], Tj + ebp);
        esi := save1;
        edi := save2;
        ebp := save3;
        ebx := save4;
  ...
  ensures  RExtend(old($r2), $r2);
{
  ...
  if (eax != ?OTHER_VECTOR_TAG) { goto skip5; }
    ...
    //if (!SupportedVectorStruct(esi, edi)) { goto vecSkip2; }
    if (esi != 8) { goto case1; }
    if (edi != 17) { goto case1; }
    goto scan;
    case1:
    if (esi != 12) { goto case2; }
    if (edi != 529) { goto case2; }
    goto scan;
    case2:
    if (esi != 16) { goto case3; }
    if (edi != 17) { goto case3; }
    goto scan;
    case3:
    if (esi != 20) { goto case4; }
    if (edi != 81) { goto case4; }
    goto scan;
    case4:
    if (esi != 24) { goto vecSkip2; }
    if (edi != 6636321) { goto vecSkip2; }
    scan:
    ...
    vecSkip2:
      assert !(
          (ebp != ?TYPE_STRUCT || (ebp == ?TYPE_STRUCT && edi == ?SPARSE_TAG))
       || (ebp == ?TYPE_STRUCT && SupportedVectorStruct(esi, edi)));
      call DebugBreak();
