...
//For the moment, we only support these vectors of structs with pointers:
//  System.NativeDelegateRecord 8 17  (0x11) --> pointers at 0
//  System.Collections.bucket  12 529 (0x211) --> pointers at 0, 1
//  Crafty.Net.OO.CHESS_PATH   16 17  (0x11) --> pointers at 0
//  Bartok.Ir.OperatorInfo     20 81  (0x51) --> pointers at 4
//  Microsoft.Zing.ProcessInfo 24 6636321 (0x654321) --> pointers at 1, 2, 3, 4, 5
// future work: support all vectors and arrays of structs with pointers
function SupportedVectorStruct(size:int, mask:int) returns(bool)
{
    (size == 8 && mask == 17)
 || (size == 12 && mask == 529)
 || (size == 16 && mask == 17)
 || (size == 20 && mask == 81)
 || (size == 24 && mask == 6636321)
}

// off = j - baseWords
// REVIEW: at least prove these from one general case, rather than assuming each individually
function SupportedVectorStructPointer(size:int, mask:int, off:int) returns(bool)
{
    (size == 8 && mask == 17 && and(off, 1) == 0)
 || (size == 12 && mask == 529 && off % 3 != 2)
 || (size == 16 && mask == 17 && and(off, 3) == 0)
 || (size == 20 && mask == 81 && off % 5 == 4)
 || (size == 24 && mask == 6636321 && off % 6 != 0)
}
...
(
 ...
 && (tag(vt) == ?SPARSE_TAG ==>
        (forall j:int::{TO(j)} TO(j) ==>
         ...
         && (fieldToSlot(vt, j) != 0 ==> between(1, 8, fieldToSlot(vt, j))
             && getNib(mask(vt), 4 * fieldToSlot(vt, j)) + 1 == j))
     && (forall k:int::{TSlot(k)} TSlot(k) ==> 1 <= k && k < 8 && getNib(mask(vt), 4 * k) != 0 ==>
              between(0, numFields(abs), getNib(mask(vt), 4 * k) + 1)
 ...
 && (tag(vt) == ?OTHER_VECTOR_TAG ==>
     ...
     && (arrayOf(vt) == ?TYPE_STRUCT && mask(arrayElementClass(vt)) == ?SPARSE_TAG ==>
          (forall j:int::{TO(j)} TO(j) ==> !VFieldPtr(abs, j)))
     && (arrayOf(vt) == ?TYPE_STRUCT && SupportedVectorStruct(arrayElementSize(vt), mask(arrayElementClass(vt))) ==>
          (forall j:int::{TO(j)} TO(j) ==> (VFieldPtr(abs, j) <==>
              baseWords(vt) <= j && j < numFields(abs)
           && SupportedVectorStructPointer(arrayElementSize(vt), mask(arrayElementClass(vt)), j - baseWords(vt)))))
    )
